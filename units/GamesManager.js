...
class GamesManager {
		...
		try {
			...
			const deadWhere = {
				where: {
					createdAt: {
						[Op.lt]: new Date(
							Date.now() - 1000 * 60 * ContestPlayer.whaitingTime
						)
							.toISOString()
							.replace('T', ' '),
					},
	...
	static async checkNeededPlayersCount(game) {
		...
		if (playersInGame < 3) return false

		// Если конструктор
		if (game.gametypeId == Game.types.CONSTRUCTOR) {
			// Игроков должно быть больше, чем задано ролей
			const needCount = await GameInitRole.rolesCount(game.id)
			if (needCount > playersInGame) return false
		}

		// Админ может запускать любые игры
		if (game.account && game.account.role == 1) return true

		// Если игроков больше или равно минимально допустимому количеству - возвращаю true
		if (playersInGame >= minCount || game.gametypeId == Game.types.CONSTRUCTOR)
			return true
