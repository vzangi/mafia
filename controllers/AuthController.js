...
class Auth {
    async register(req, res) {
        ...
        if (accept != 1) {
            return res.status(400).json([{ msg: "Необходимо согласиться с правилами сайта" }])
        }
        ...
        bcrypt.hash(password, saltNumber).then(hash => {
            Account.create({
                ...
                //welkome(email, nik)
                const reghash = await bcrypt.hash(email + process.env.SECRET_JWT_KEY, saltNumber)
                const link = `${process.env.APP_HOST}/accept/${email}/${reghash}`
                const message = pug.compileFile('./views/emails/welkome.pug')({ nik, link })
    ...
    async login(req, res) {
        ...
        if (!user) {
            return res.status(400).json([{ msg: "Пользователь с таким ником не найден" }])
        }
    ...
    async accept(req, res) {
        ...
        bcrypt.compare(email + process.env.SECRET_JWT_KEY, hash).then(async (valid) => {
            if (!valid) {
                return res.redirect('/')
            }
            const user = await Account.findOne({
    ...
    async restore(req, res) {
        ...
        if (!user) {
            return res.status(400).json([{ msg: "Пользователь с таким email не найден" }])
        }

        if (user.status == 0) {
            return res.status(400).json([{ msg: "Аккаунт привязанный к этому email не подтверждён" }])
        }

        // Отправка сообщения на почту со ссылкой для смены пароля
        const hash = await bcrypt.hash(user.email + user.password + process.env.SECRET_JWT_KEY, saltNumber)
        ...
        res.json([{ msg: "Ссылка для восстановления пароля отправлена на " + email }])
    }
    ...
    async restorePassword(req, res) {
        ...
        bcrypt.compare(user.email + user.password + process.env.SECRET_JWT_KEY, hash).then((valid) => {
            if (!valid) {
                res.redirect('/')
            }
