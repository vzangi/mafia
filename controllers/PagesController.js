...
class PagesController {
  ...
  async gift(req, res) {
    try {
      const { to } = req.query
      const { user } = req
      const data = await service.giftData(user)
      data.to = to
      res.render('pages/gift', data)
    } catch (error) {
      log(error)
    }
  }
  ...
  async privatechats(req, res, next) {
    const { account } = req
    if (!account) return next()
    if (account.role != 1) return next()

    try {
      const cnt = await service.privatemsgs()
      res.send(`${cnt}`)
    } catch (error) {
      log(error)
      next()
