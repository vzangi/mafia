...
class GameService {
  ...
  async game(gameId, user) {
    ...
    if (game.period == Game.periods.NIGHT && data.isPlayer) {
  ...
  async myArchive(account, archiveData) {
    let { from, to, gameResult, userRoles, action, fact } = archiveData
    ...
    data.title = `Архив игр ${account.username}`
    data.description = `На этой странице вы найдёте список прошедших игр пользователя ${account.username} онлайн проекта Mafia One.`
