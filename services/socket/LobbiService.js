...
class LobbiService extends BaseService {
  ...
  async makeGame(settings) {
    ...
    if (
      ...
      gametypeId > Game.types.CONSTRUCTOR ||
      gametypeId == 3
    ) {
  ...
  async addToContest(data) {
    ...
    log(`> ${user.id} contests: ${JSON.stringify(contests)}`)
    // console.log('>', user.id, 'contests: ', contests)
    ...
    // Сообщаю игроку, что заявка в очереди
    this.getUserSockets(user.id, '/lobbi').forEach((sock) =>
      sock.emit('make.contest')
    )
  ...
  async leaveGame(gameId) {
    ...
    if (game.seconds <= 10) {
      throw new Error('Нельзя покинуть заявку за 10 секунд до начала игры')
    }
  ...
  async claim(data) {
    ...
    if (player.id == 1) {
      console.log(`Жалоба на админа от: ${account.username}`)
      throw new Error('Жалоба принята')
    }
