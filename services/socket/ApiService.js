...
class ApiService extends BaseService {
    ...
    async changeNik(newnik) {
        ...
        if (!nik.match(nikPattern)) {
            throw new Error("Ник может состоять только из букв, цифр, дефисов и знаков нижнего подчеркивания")
        }
        ...
        if (account.wallet < price) {
            throw new Error(`Для смены ника на счету должно быть как минимум ${price} рублей`)
        }
