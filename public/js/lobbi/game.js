$(function () {
	...
	function removeGame(game) {
		$(game).find('.btn-to-game').remove()
		$(game).addClass('removing')
		setTimeout(function () {
			$(game).remove()
		}, 500)
	}
	...
	lobbiSocket.on('game.remove', (id) => {
		...
		removeGame($(`.game-item[data-id=${id}]`))
	})
	...
	lobbiSocket.on('game.play', (id) => {
		if ($('.playsound').length == 1) playSound(startSound)
	...
	lobbiSocket.on('game.player.add', (gameId, player) => {
		...
		setTimeout(() => $(`.player.added`).removeClass('added'), 50)
	...
	function showGame(game) {
		...
		setTimeout(
			() => $(`.game-item[data-id='${game.id}']`).removeClass('removing'),
			10
		)
